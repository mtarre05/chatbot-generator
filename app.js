    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                button { padding: 10px; font-size: 16px; background: #f86c30; color: white; border: none; cursor: pointer; }
                button:hover { background: #c2d5fd; }
            </style>
        </head>
        <body>
            <h1>Generate Chatbot HTML</h1>
            <p>Paste your Flowise embed code below and click 'Generate'.</p>
            <form action="/generate" method="post">
                <label for="flowiseScript">Flowise Embed Code:</label>
                <textarea id="flowiseScript" name="flowiseScript" required placeholder='<script type="module">...</script>'></textarea>
...
app.post('/generate', async (req, res) => {
    const flowiseScript = req.body.flowiseScript;
    let chatflowId = '';
    let apiHost = '';

    // Regex to find chatflowid and apiHost
    const chatflowIdMatch = flowiseScript.match(/chatflowid:\s*"([^"]+)"/);
    const apiHostMatch = flowiseScript.match(/apiHost:\s*"([^"]+)"/);

    if (chatflowIdMatch && apiHostMatch) {
        chatflowId = chatflowIdMatch[1];
        apiHost = apiHostMatch[1];
    }

    if (!chatflowId || !apiHost) {
        return res.status(400).send('Error: Could not find valid chatflowid and apiHost in the provided script. Please ensure the code is correct.');
    }
